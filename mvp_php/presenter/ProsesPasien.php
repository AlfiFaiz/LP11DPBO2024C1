...
class ProsesPasien implements KontrakPresenter
{
	...
	function prosesDataPasien()
	{
		try {
			...
			while ($row = $this->tabelpasien->getResult()) {
				...
				$pasien->setGender($row['email']); //mengisi gender
				$pasien->setGender($row['telp']); //mengisi gender
	...
	function hapusDataPasien($id) {
        // Call the deletePasien method from TabelPasien class
        return $this->tabelpasien->deletePasien($id);
    }
	function getDataById($id) {
	
			// Mengambil hasil query
			return $this->tabelpasien->getDataById($id);
